...
async function ebaySearch(
  ...
  if (activeOnly) {
    ...
    // soldItems:true required for actual sold item data + correct sold URLs.
    // itemEndDate without soldItems returns relisted active items (wrong URLs).
    // itemEndDate WITH soldItems is silently ignored by eBay.
    // Post-fetch filters handle date/active exclusion: dropStale() + sold/active split.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const _TCG_RE = /\b(pokemon|pokémon|pikachu|charizard|mewtwo|eevee|bulbasaur|squirtle|gengar|mtg|magic|yugioh|yu-gi-oh|lorcana|dragon\s*ball|dbs|one\s*piece|digimon|agumon|gabumon|greymon|wargreymon|omnimon|omegamon|gallantmon|imperialdramon|patamon|gatomon|veemon|guilmon|renamon|terriermon|myotismon|devimon|beelzemon|gundam|mobile\s*suit|gundanium|zaku|rx-78|newtype|union\s*arena|unionarena|bleach|ichigo|jujutsu\s*kaisen|jjk|hunter\s*x\s*hunter|hxh|code\s*geass|lelouch|fortuneteller\s*baba|master\s*roshi|oolong|puar|ox.king|chichi|chi.chi|launch|turtle\s*hermit|kame|yamcha|tien|chiaotzu|raditz|nappa|zarbon|dodoria|ginyu|recoome|burter|jeice|guldo)\b/i

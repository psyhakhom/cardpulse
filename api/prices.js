...
async function ebaySearch(
  ...
  if (live) {
    ...
    // itemEndDate range: only return items that ended (sold) within the window.
    // Active listings have no past itemEndDate — excluded at API level.
    // dropStale() + parallel sold/active split remain as safety nets.
    const now = new Date()

...
async function ebaySearch(
  ...
  if (live) {
    ...
    // itemEndDate filter is silently ignored by eBay when combined with soldItems:true
    // (confirmed: active listings with no itemEndDate still returned).
    // Active listing exclusion handled post-fetch: dropStale() for normal path,
    // sold/active split for parallel path.

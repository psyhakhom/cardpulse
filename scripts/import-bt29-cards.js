/**
 * Import DBS Classic BT29 "Fearsome Rivals" (UB02) cards from the official
 * Bandai US-EN card game site into card_catalog with proper card names,
 * rarities, and correct Bandai US-EN image URLs.
 *
...
const SET_CODE = 'BT29'
const SET_ID = 428029
const SET_NAME = 'Fearsome Rivals'
...
async function fetchSetCards() {
  ...
  // Try alt text first (most reliable: alt="BT29-001 Son Goku")
  const altRe = /alt="(BT29-\d{3}[A-Z]?(?:_(?:PR|SPR|SCR))?)\s+([^"]+)"/gi
  let m
  ...
  if (cards.length === 0) {
    const tripleRe = /class="cardNumber"[^>]*>\s*(BT29-\d{3}[A-Z]?(?:_\w+)?)\s*<\/dt>\s*<dd class="cardName"[^>]*>\s*([^<]+)/gi
...
async function main() {
  console.log(`Starting ${SET_CODE} "${SET_NAME}" import from dbs-cardgame.com...`)
  ...
  console.log(`\n${'═'.repeat(50)}`)
  console.log(`  ✓ Done — ${cards.length} ${SET_CODE} cards imported`)

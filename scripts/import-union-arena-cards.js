/**
 * Bulk import Union Arena TCG cards from apitcg.com
 ...
 *   3. Run: npm run import-union-arena
 *
 * API response shape (www.apitcg.com/api/union-arena/cards):
 *   { page, limit, total, totalPages, data: [ { id, code, name, rarity,
 *     type, bp, affinity, effect, trigger, images: { small, large },
 *     set: { name } } ] }
 *
 * ~541 cards total. No set.id field — set_code derived from card code prefix.
 */
...
async function fetchPage(page) {
  const url = `https://www.apitcg.com/api/union-arena/cards?limit=100&page=${page}`
...
function mapCard(card) {
  ...
  // Union Arena codes like "UA01BT/JJK-1-001" — extract set prefix before first dash-number
  const setMatch = code.match(/^([A-Z0-9]+(?:\/[A-Z]+-\d+)?)/)
  const setCode = setMatch ? setMatch[1] : code.replace(/-\d+$/i, '') || null
  return {
    card_name: card.name || '',
    ...
    search_query: `${card.name || ''} ${code} ${rarity}`.trim(),
    times_searched: 0,
...
async function main() {
  console.log('Starting Union Arena import from apitcg.com...')
  ...
  for (let page = 2; page <= totalPages; page++) {
    ...
    try {
      ...
      console.log(`Page ${page}: fetched ${cards.length} cards, total: ${totalImported}`)
      if (batch.length >= 50) { await flushBatch(batch); batch = [] }
    } catch (err) {
  ...
  console.log(`\n${'═'.repeat(50)}`)
  console.log(`  ✓ Done — ${totalImported} Union Arena cards imported`)

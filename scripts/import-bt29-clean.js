/**
 * Clean import of BT29 "Fearsome Rivals" (UB02) from Bandai US-EN.
 *
 * Deletes ALL existing BT29 rows from card_catalog first, then re-imports
 * from scratch using the authoritative Bandai US-EN source. This ensures
 * no bad Asia/Deckplanet data survives.
 *
 * Source: https://www.dbs-cardgame.com/us-en/cardlist/ (POST with category_exp=428029)
 *
 * Prerequisites:
 *   .env.local with SUPABASE_URL and SUPABASE_SERVICE_KEY
 *
 * Usage: npm run import-bt29-clean
 */
...
async function fetchSetCards() {
  ...
  // Try alt text first (most reliable: alt="BT29-001 Krillin")
  const altRe = /alt="(BT29-\d{3}[A-Z]?(?:_(?:PR|SPR|SCR|GDR))?)\s+([^"]+)"/gi
  let m
...
async function main() {
  ...
  // Step 2: Delete all existing BT29 rows
